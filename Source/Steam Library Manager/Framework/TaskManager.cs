...
namespace Steam_Library_Manager.Framework
{
    internal static class TaskManager
    {
            ...
            try
            {
                ...
                if (CurrentTask.SteamApp != null)
                {
                    ...
                    if (!CancellationToken.IsCancellationRequested && !CurrentTask.ErrorHappened)
                    {
                        ...
                        {
                            Main.FormAccessor.TaskManager_Logs.Add(string.Format(Functions.SLM.Translate(Properties.Resources.TaskManager_RemoveOldFiles), DateTime.Now, CurrentTask.SteamApp.AppName));
                            await CurrentTask.SteamApp.DeleteFilesAsync(CurrentTask);
                            CurrentTask.SteamApp.Library.Steam.Apps.Remove(CurrentTask.SteamApp);
                            Main.FormAccessor.TaskManager_Logs.Add(string.Format(Functions.SLM.Translate(Properties.Resources.TaskManager_RemoveOldFilesCompleted), DateTime.Now, CurrentTask.SteamApp.AppName));
                        }
                        ...
                        CurrentTask.TaskStatusInfo = Functions.SLM.Translate(Properties.Resources.TaskStatus_Completed);
                ...
                else if (CurrentTask.OriginApp != null)
                {
                    ...
                    if (!CancellationToken.IsCancellationRequested && !CurrentTask.ErrorHappened)
                    {
                        if (CurrentTask.RemoveOldFiles && CurrentTask.TaskType != Definitions.Enums.TaskType.Delete)
                        {
                            Main.FormAccessor.TaskManager_Logs.Add(string.Format(Functions.SLM.Translate(Properties.Resources.TaskManager_RemoveOldFiles), DateTime.Now, CurrentTask.OriginApp.AppName));

                            CurrentTask.OriginApp.DeleteFiles(CurrentTask);
                            CurrentTask.OriginApp.Library.Origin.Apps.Remove(CurrentTask.OriginApp);

                            Main.FormAccessor.TaskManager_Logs.Add(string.Format(Functions.SLM.Translate(Properties.Resources.TaskManager_RemoveOldFilesCompleted), DateTime.Now, CurrentTask.OriginApp.AppName));
                        }
        ...
        public static void Start()
        {
            if (!Status && !Paused)
            {
                Main.FormAccessor.TaskManager_Logs.Add(string.Format(Functions.SLM.Translate(Properties.Resources.TaskManager_Active), DateTime.Now));
                ...
                Task.Factory.StartNew(async () =>
                {
                    ...
                    {
                        Stop();
                        Main.FormAccessor.TaskManager_Logs.Add(string.Format(Functions.SLM.Translate(Properties.Resources.TaskManager_Stopped), DateTime.Now));
                    }
                    catch (Exception ex)
                    {
                        Debug.WriteLine(ex);
        ...
        public static void Pause()
        {
            try
            {
                if (Status && ActiveTask != null)
                {
                    ...
                    Main.FormAccessor.TaskManager_Logs.Add(string.Format(Functions.SLM.Translate(Properties.Resources.TaskManager_Paused), DateTime.Now));
                }
            }
            catch (Exception ex)
            {
